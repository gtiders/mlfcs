...
- **Breaking:** the documentation site no longer serves the `/mlfcs/en/` prefix.
  ...
  segment must drop it.
- Documentation sources are flat: `docs/*.md` is English and `docs/zh/*.md` its Chinese mirror. Per
  page language alternates, the language selector, and localized navigation are generated by
  `mkdocs-static-i18n`, and the site shows the project logo above the primary navigation.
- The documentation site now uses one flat paper colour for the page, the header, and the sidebar
  brand block, with a matching dark scheme; surfaces are separated by hairline rules instead of a
  coloured header.
- The documentation tooling lives under `docs/`: `docs/overrides/` holds the theme overrides and
  `docs/scripts/` the site checks. Neither is published with the site.
