...
- 文档站点改用单一浅色纸面色：页面、上边栏与侧栏品牌块同色，暗色模式配套同色深底，各层之间以
  发丝线分隔，不再使用彩色上边栏。
- 文档工具迁入 `docs/`：`docs/overrides/` 存放主题覆盖，`docs/scripts/` 存放站点检查脚本，两者都
  不会随站点发布。
...
- 新增 `SymmetryViolationError`,并在 `LoopSCPH`、`HarmonicSampler` 与 `harmonic_frequencies` 上提供
  显式的 `symmetry_tolerance`:每个倒空间消费者在展开之前都会检查力常数是否在每个星代表点的
  little group 上协变,违反时报告操作编号、标签、残差与尺度,而不是把它平均掉,并把该容差记录在结果与
  元数据中。
...
- 晶体对称性检查默认开启,因此手工构造或严重截断、不满足自身空间群的模型现在会被拒绝,并在消息中
  给出违规操作与标签。此类测试夹具可用 `symmetry_tolerance=None` 显式关闭检查;spglib 无法确定对称性的
  结构现在会报告原子数、cell 与 `symprec`。
