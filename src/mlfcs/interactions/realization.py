...
from mlfcs.interactions.algebra.actions import apply_scaled_columns
...
@cache
def _image_matrix(
    rotation: tuple[int, ...], permutation: tuple[int, ...], order: int
) -> np.ndarray:
    """Return the exact integer lattice-frame action matrix of one orbit image."""
    return apply_scaled_columns(
        np.asarray(rotation, dtype=np.int64).reshape(3, 3),
        permutation,
        order,
        np.eye(3**order, dtype=np.int64),
    )
...
def validate_realization_identifiability(
    ...
    *,
    ...
    Every coefficient is an exact integer: the orbit basis is its lattice (scaled)
    form, tensor actions are the integer lattice rotations, and each column-graph
    component is ranked exactly.  No coefficient filter and no rank tolerance is
    involved, so a genuinely non-zero coefficient can never be discarded.  Lattice
    coordinates are integral for every cell, including cells whose Cartesian
    rotations are irrational.
    """
    ...
    for orbit_index, orbit in enumerate(space.orbits):
        basis = np.ascontiguousarray(orbit.basis, dtype=np.int64)
        ...
        for image_index, image in enumerate(orbit.images):
            ...
            action = image.action
            scaled_rotation = getattr(action, "scaled_rotation", None)
            if scaled_rotation is None:
                raise RuntimeError("orbit image carries no lattice-frame rotation")
            matrix = _image_matrix(
                tuple(int(value) for value in np.asarray(scaled_rotation).reshape(-1)),
                action.permutation,
                space.order,
            )
            integer_columns = matrix @ basis
            for component, values in enumerate(integer_columns):
    ...
    for root, columns in components.items():
        ...
        if len(columns) == 1:
            rank = 1 if any(values.get(columns[0]) for values in local_rows) else 0
        else:
...
def _realized_cluster(
    image, orbit_index: int, image_index: int, index, realized
) -> tuple[int, ...]:
    if realized is None:
...
def _realize_orbit_space(space: PrimitiveInteractionSpace, index) -> RealizedInteractionSpace:
    ...
    return RealizedInteractionSpace(
        space.order, tuple(realized), space.cell, space.cutoff, space.max_body_order
    )
