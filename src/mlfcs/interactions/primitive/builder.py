...
def primitive_generators(
    ...
    """Return the orbit generators in both frames.

    The lattice rotation is spglib's integer matrix and the Cartesian rotation is derived
    from it through the cell, so the two frames of a generator cannot disagree and the
    frame convention lives in ``scaled_to_cartesian_rotation`` alone.
    """
...
def generated_orbit(
    ...
    *,
    ...
    """Build one orbit with its invariant basis and parameters in the lattice frame.

    The invariant subspace, its dimension, the basis and the parameter values all come
    from exact integer arithmetic on the lattice (scaled) rotations, which are integral
    for every cell.  ``frame`` is only used to pick the components that a
    finite-difference plan observes and that the fitted tensors are rendered from.
    """
    ...
    kernel, dimension = invariant_kernel(label_basis, result.stabilizers, order=seed.order)
    basis = result.seed_to_canonical.apply_scaled_columns(label_basis @ kernel)
    pivots = select_independent_rows(frame @ basis, dimension=dimension)
