...
def reconstruct_sparse(
    ...
    *,
    ...
    for orbit in orbit_space.orbits:
        ...
        # The parameters are the coefficients of the orbit basis, so the observed pivot
        # components determine them through the Cartesian image of that block.
        block = (frame @ orbit.basis)[orbit.pivots]
        coefficients.append(np.linalg.solve(block, np.asarray(observed)))

    original_parameters = np.concatenate(coefficients) if coefficients else np.empty(0, dtype=float)

    if enforce_asr:
        coefficient_space = primitive_interaction_space or orbit_space
        coefficients, initial_drift, final_drift = project_acoustic_sum_rule(
            coefficient_space, coefficients, return_drift=True
        )
