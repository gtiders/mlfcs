...
def test_harmonic_sampler_and_fourier_kernel_agree(case_name, supercell):
    ...
    matrix_from_lattice = np.asarray([dynamical_matrix(terms, masses, q) for q in qpoints])
    matrix_from_compact = np.asarray([sampler._dynamical_matrix(q) for q in qpoints])
