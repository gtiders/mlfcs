...
则先取复共轭。在 $\Gamma$ 点，模态空间是**内部**子空间：三个质量加权平移是平移算符的零空间，
在本征问题之前就被投影掉，协方差随后再抬回全空间。因此它们不是以 $1/\omega^2$ 的权重出现在
$W(\Gamma)$ 中，而是根本不出现；平移不变模型的 $\Gamma$ 协方差无论是否设置频率 cutoff 都是有限的。
简并子空间内本征基的列是一种规范，因此跨星比较的只能是投影算符、频率与协方差，而不是列本身。频率与任何谱量在整个星上相同，因此完整网格由代表点数据**精确展开**得到，
...
协方差门禁比动力学矩阵门禁更严格：权重按 $1/\lambda$ 放大，因此矩阵门禁在给定相对容差下接受的残差，
会被最大与最小被包含本征值之比放大，展开后的协方差可能与成员自身 q 点上直接构造的协方差相差超过同一容差。
求解器逐成员检查协方差本身，而不是从矩阵的协方差去推断。软模或虚频模会在其所在位置被报告——
q 点、模编号、本征值、频率、温度——除非调用方显式要求某种策略，否则不会被 $\sqrt{\lvert\lambda\rvert}$ 转成稳定模。
...
| 体系 | $N_q$ | $N_{\mathrm{irr}}$ | 约化比 | 频率对角化 | 单轮 SCPH 对角化 | 采样器初始化对角化 |
|---|---|---|---|---|---|---|
| diamond 2×2×2 | 64 | 8 | 8.00 | 8（全网格 64） | 9（全网格 24） | 3（全网格 8） |
| hcp 2×2×2 | 64 | 12 | 5.33 | 12（全网格 64） | 12（全网格 24） | 4（全网格 8） |
| GaAs 3×2×2 | 96 | 34 | 2.82 | 34（全网格 96） | 18（全网格 36） | 6（全网格 12） |

自由能与采样本身不再进入对角化器：报告里的谐波自由能耗时为 $10^{-4}$ s 量级且对角化次数为 0，
4096 个快照的单批采样为 $2\text{–}4\times10^{-2}$ s。

在展开之前,每个真正被约化展开的力常数都会先通过**全星协变门禁**:门禁用代表点矩阵按
$W_m=\Gamma_m U_m\overline{W_s}^{[a_m]}U_m^\dagger\Gamma_m^\dagger$ 展开,再与在每个成员自身标签上直接构造的
动力学矩阵逐成员比较。little group 条件只约束代表点本身(它按定义固定代表点),因此既不是充分条件,
也不能作为合并门禁;它只保留为定位稳定子群问题的局部诊断。容差是**相对**量:允许残差为
$\texttt{symmetry\_tolerance}\cdot s$,$s$ 取直接与展开两组矩阵的最大无穷范数。`symmetry_tolerance=None`
是唯一关闭门禁的方式;`NaN`、`+inf`、负值一律拒绝,因为它们会让比较恒为假而静默关闭门禁。`symprec`
必须有限且严格为正。

门禁会构造完整网格的 $D(q)$,但只有 $N_{\mathrm{irr}}$ 个矩阵进入 `eigh/eigvalsh`;基准必须同时报告
matrix builds、eigensolver matrices、星展开和完整 Fourier 求和,不能只报对角化约化。
